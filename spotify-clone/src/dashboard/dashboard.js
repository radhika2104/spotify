...
const loadPlaylist = async (endpoint, elementID) => {
  ...
  const {
    playlists: { items },
  } = featuredPlaylistInfo;
  // console.log('items:',items)
  for (let item of items) {
    const { images, name, description, id } = item;
    const playlistItem = document.createElement("section");
    playlistItem.className =
      "bg-black-secondary rounded p-4 hover:cursor-pointer hover:bg-light-black transition duration-150 hover:ease-in";
    playlistItem.id = id;
    playlistItem.setAttribute("data-type", "playlist");
    playlistItem.addEventListener("click", () =>
      navigateToSelectedPlaylist(id)
    );
    playlistItem.innerHTML = `   <img src="${images[0].url}" alt="${name}" class="rounded mb-2 object-contain drop-shadow">
...
const fillDashboardMainContent = () => {
  ...
  coverContent.innerHTML = `<h1 class="text-6xl">Hello ${
    displayName.split(" ")[0]
  }!</h1>`;
  const pageContent = document.getElementById("page-content");
  const contentMap = new Map([
    ["Featured", "featured-playlist-items"],
    ["Top Playlists", "toplist-playlist-items"],
  ]);
  let innerHTMLdata = "";
  for (let [name, id] of contentMap) {
    innerHTMLdata += `<article class="p-4">
...
const trackClickHandler = (event) => {
  ...
  document.querySelectorAll(".track").forEach((trackItem) => {
    ...
    if (
      trackItem.contains(event.target) &&
      ...
      const [imageURL, artistNames, title, duration, id, previewURL] =
        playBtnTrackDetails.split("@,");
      playTrackHandler([
        imageURL,
        artistNames,
        title,
...
function updateIconsForPlayMode(selectedTrackID) {
  ...
  const playBtnFromTracks = document.querySelector(
    `#play-track-${selectedTrackID}`
  );
...
const loadPlaylistTracks = async (playlist_id) => {
  ...
  const playlistTracksData = await fetchRequest(
  ...
  for (let trackItem of trackItems.filter((item) => item.track.preview_url)) {
    ...
    tracksSection.innerHTML += `<article id="${id}" class="track p-1 grid grid-cols-[50px_1fr_1fr_50px] gap-4 justify-items-start items-center hover:bg-light-black  rounded-md">
        <p class="relative w-full flex items-center justify-center text-secondary">
        <span class="track-no">${trackCounter}</span>
        <button trackdetails="${
          image.url
        }@,${artistNames}@,${title}@,${duration}@,${id}@,${previewURL}" id="play-track-${id}" class="play absolute w-full invisible text-primary material-symbols-outlined">play_arrow</button>
        </p>
...
const scrollEventHandler = (event) => {
  ...
  const coverOpacity =
    100 - (scrollTop >= ccHeight ? 100 : (scrollTop / ccHeight) * 100);
...
function getCurrentTrack() {
  ...
  if (currentTrackPlayingID) {
    ...
    const currentTrackIndex = tracksArrayFromStorage?.findIndex(
      (trackItem) => trackItem[4] === currentTrackPlayingID
    );
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  audio.addEventListener("play", () => {
    ...
    const selectedTrack = tracksSection?.querySelector(
      `[id="${selectedTrackID}"]`
    );
